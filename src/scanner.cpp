...
std::vector<PluginManifest> Scanner::scanDirectory(const std::string& path) {
    ...
    for (const auto& entry : fs::directory_iterator(path)) {
        if (entry.is_directory()) {
            ...
            if (dirName.size() > 6 && dirName.substr(dirName.size() - 6) == ".clasp") {
...
std::optional<PluginManifest> Scanner::loadManifest(const std::string& bundlePath) {
    std::string jsonPath = bundlePath + "/plugin.json";
    return parseManifest(jsonPath, bundlePath);
}

std::optional<PluginManifest> Scanner::parseManifest(const std::string& jsonPath,
    ...
    try {
        ...
        if (!json.hasObjectMember("id") || !json.hasObjectMember("name") ||
            ...
            std::cerr << "[clasp] Missing required fields in: " << jsonPath << std::endl;
            return std::nullopt;
        }
        ...
        std::cerr << "[clasp] JSON parse error in " << jsonPath << ": " << e.what() << std::endl;
        return std::nullopt;
    }
}
