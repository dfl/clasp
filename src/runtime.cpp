...
bool Runtime::isCacheValid(const std::string& wasmPath, const std::string& cachePath) {
    if (!fs::exists(cachePath)) return false;
    if (!fs::exists(wasmPath)) return false;
...
bool Runtime::saveToCache(wasmtime_module_t* module, const std::string& cachePath) {
...
wasmtime_module_t* Runtime::loadFromCache(const std::string& cachePath) {
    ...
    wasmtime_error_t* error = wasmtime_module_deserialize(engine_, bytes.data(),
                                                           bytes.size(), &module);
    if (error) {
        wasmtime_error_delete(error);
        return nullptr;
...
wasmtime_module_t* Runtime::compileOrLoadCached(const std::string& wasmPath,
    ...
    if (aotCacheEnabled_ && !cacheDir_.empty()) {
        ...
        if (isCacheValid(wasmPath, cachePath)) {
            ...
            if (module) {
                std::cerr << "[clasp] Loaded AOT-cached module: " << wasmPath << std::endl;
                return module;
            }
        }
    }
    ...
    wasmtime_error_t* error = wasmtime_module_new(engine_, wasmBytes.data(),
                                                   wasmBytes.size(), &module);
    if (error) {
...
std::unique_ptr<WasmInstance> Runtime::loadModule(const std::string& wasmPath) {
    ...
    wasmtime_error_t* error = wasmtime_linker_instantiate(linker, context_, module, &instance, &trap);
    ...
    bool found = wasmtime_instance_export_get(context_, &instance, "memory", 6, &memoryExtern);
    ...
    auto getFunc = [&](const char* name, wasmtime_func_t* func) -> bool {
        ...
        if (!wasmtime_instance_export_get(context_, &instance, name, strlen(name), &ext)) {
            return false;
        }
...
WasmInstance::WasmInstance(wasmtime_context_t* context, wasmtime_instance_t instance,
...
bool WasmInstance::callVoid(wasmtime_func_t* func) {
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, func, nullptr, 0, nullptr, 0, &trap);
...
bool WasmInstance::callVoidInt(wasmtime_func_t* func, int32_t arg) {
    ...
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, func, args, 1, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
    return !error && !trap;
}
...
bool WasmInstance::callVoidFloatInt(wasmtime_func_t* func, float arg1, int32_t arg2) {
    wasmtime_val_t args[2] = {
        {.kind = WASMTIME_F32, .of = {.f32 = arg1}},
        {.kind = WASMTIME_I32, .of = {.i32 = arg2}}
    };
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, func, args, 2, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
    return !error && !trap;
}

bool WasmInstance::callVoidIntFloat(wasmtime_func_t* func, int32_t arg1, float arg2) {
    wasmtime_val_t args[2] = {
        {.kind = WASMTIME_I32, .of = {.i32 = arg1}},
...
int32_t WasmInstance::callInt(wasmtime_func_t* func) {
    ...
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, func, nullptr, 0, result, 1, &trap);
    if (error) { wasmtime_error_delete(error); return 0; }
...
int32_t WasmInstance::callIntInt(wasmtime_func_t* func, int32_t arg) {
    ...
    wasmtime_val_t result[1];
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, func, args, 1, result, 1, &trap);
    if (error) { wasmtime_error_delete(error); return 0; }
    if (trap) { wasm_trap_delete(trap); return 0; }
    return result[0].of.i32;
}
...
float WasmInstance::callFloatInt(wasmtime_func_t* func, int32_t arg) {
    ...
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, func, args, 1, result, 1, &trap);
    if (error) { wasmtime_error_delete(error); return 0.0f; }
    if (trap) { wasm_trap_delete(trap); return 0.0f; }
...
int WasmInstance::getStateSize() {
    return callInt(&funcs_.getStateSize);
}
...
void WasmInstance::getState(uint8_t* out) {
    ...
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, &funcs_.getState,
                                                  args, 1, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
...
void WasmInstance::setState(const uint8_t* in, int size) {
    ...
    wasmtime_val_t args[1] = {{.kind = WASMTIME_I32, .of = {.i32 = 0}}};
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, &funcs_.setState,
                                                  args, 1, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
}
...
void WasmInstance::noteOn(int32_t sampleOffset, int16_t noteId, int16_t channel,
    ...
    if (!funcs_.hasInstrumentSupport) return;
    ...
    wasmtime_val_t args[5] = {
        {.kind = WASMTIME_I32, .of = {.i32 = sampleOffset}},
    ...
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, &funcs_.noteOn,
                                                  args, 5, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
}

void WasmInstance::noteOff(int32_t sampleOffset, int16_t noteId, int16_t channel,
                           int16_t key, float velocity) {
    if (!funcs_.hasInstrumentSupport) return;
    ...
    wasm_trap_t* trap = nullptr;
    wasmtime_error_t* error = wasmtime_func_call(context_, &funcs_.noteOff,
                                                  args, 5, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
}
