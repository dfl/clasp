...
static void ensureScanned() {
    ...
    for (auto& manifest : g_manifests) {
        ...
        static const char* effectFeatures[] = {
            CLAP_PLUGIN_FEATURE_AUDIO_EFFECT,
            nullptr
        };
        static const char* instrumentFeatures[] = {
...
public:
    ...
    bool paramsGetInfo(uint32_t index, clap_param_info_t* info) const {
        if (index >= manifest_.parameters.size()) return false;
    ...
    bool paramsValueToText(clap_id paramId, double value, char* display, uint32_t size) {
    ...
    bool audioPortsGet(uint32_t index, bool isInput, clap_audio_port_info_t* info) const {
        ...
        info->channel_count = isInput ? manifest_.audio.inputs : manifest_.audio.outputs;
    ...
    uint32_t notePortsCount(bool isInput) const {
        if (!manifest_.isInstrument) return 0;
        return isInput ? 1 : 0;
    }

    bool notePortsGet(uint32_t index, bool isInput, clap_note_port_info_t* info) const {
        if (!manifest_.isInstrument || index != 0 || !isInput) return false;
...
struct PluginWrapper {
    ...
    PluginWrapper(const clap_host_t* host, const clap_plugin_descriptor_t* desc,
        ...
        plugin.activate = [](const clap_plugin_t* p, double sr, uint32_t minF, uint32_t maxF) -> bool {
            return static_cast<PluginWrapper*>(p->plugin_data)->impl.activate(sr, minF, maxF);
        };
        ...
        plugin.start_processing = [](const clap_plugin_t* p) -> bool {
            return static_cast<PluginWrapper*>(p->plugin_data)->impl.startProcessing();
        };
        ...
        plugin.process = [](const clap_plugin_t* p, const clap_process_t* proc) -> clap_process_status {
...
static const clap_plugin_params_t paramsExtension = {
    ...
    .get_info = [](const clap_plugin_t* p, uint32_t idx, clap_param_info_t* info) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.paramsGetInfo(idx, info);
    },
    .get_value = [](const clap_plugin_t* p, clap_id id, double* val) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.paramsGetValue(id, val);
    },
    .value_to_text = [](const clap_plugin_t* p, clap_id id, double val, char* txt, uint32_t sz) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.paramsValueToText(id, val, txt, sz);
    },
    .text_to_value = [](const clap_plugin_t* p, clap_id id, const char* txt, double* val) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.paramsTextToValue(id, txt, val);
    },
    .flush = [](const clap_plugin_t* p, const clap_input_events_t* in, const clap_output_events_t* out) {
        static_cast<PluginWrapper*>(p->plugin_data)->impl.paramsFlush(in, out);
    },
};
...
static const clap_plugin_audio_ports_t audioPortsExtension = {
    .count = [](const clap_plugin_t* p, bool isInput) -> uint32_t {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.audioPortsCount(isInput);
    },
    .get = [](const clap_plugin_t* p, uint32_t idx, bool isInput, clap_audio_port_info_t* info) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.audioPortsGet(idx, isInput, info);
    },
};

static const clap_plugin_note_ports_t notePortsExtension = {
    .count = [](const clap_plugin_t* p, bool isInput) -> uint32_t {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.notePortsCount(isInput);
    },
    .get = [](const clap_plugin_t* p, uint32_t idx, bool isInput, clap_note_port_info_t* info) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.notePortsGet(idx, isInput, info);
    },
};

static const clap_plugin_gui_t guiExtension = {
    .is_api_supported = [](const clap_plugin_t* p, const char* api, bool floating) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiIsApiSupported(api, floating);
    },
    .get_preferred_api = [](const clap_plugin_t* p, const char** api, bool* floating) -> bool {
    ...
    .create = [](const clap_plugin_t* p, const char* api, bool floating) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiCreate(api, floating);
    },
    ...
    .set_scale = [](const clap_plugin_t* p, double scale) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiSetScale(scale);
    },
    .get_size = [](const clap_plugin_t* p, uint32_t* w, uint32_t* h) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiGetSize(w, h);
    },
    .can_resize = [](const clap_plugin_t* p) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiCanResize();
    },
    .get_resize_hints = [](const clap_plugin_t* p, clap_gui_resize_hints_t* hints) -> bool {
        return false;
    },
    .adjust_size = [](const clap_plugin_t* p, uint32_t* w, uint32_t* h) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiAdjustSize(w, h);
    },
    .set_size = [](const clap_plugin_t* p, uint32_t w, uint32_t h) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiSetSize(w, h);
    },
    .set_parent = [](const clap_plugin_t* p, const clap_window_t* win) -> bool {
        return static_cast<PluginWrapper*>(p->plugin_data)->impl.guiSetParent(win);
    },
    .set_transient = [](const clap_plugin_t* p, const clap_window_t* win) -> bool {
        return false;
    },
...
const void* PluginWrapper::getExtension(const clap_plugin_t* p, const char* id) {
    if (strcmp(id, CLAP_EXT_PARAMS) == 0) return &paramsExtension;
    if (strcmp(id, CLAP_EXT_STATE) == 0) return &stateExtension;
    if (strcmp(id, CLAP_EXT_AUDIO_PORTS) == 0) return &audioPortsExtension;
    if (strcmp(id, CLAP_EXT_NOTE_PORTS) == 0) return &notePortsExtension;
    if (strcmp(id, CLAP_EXT_GUI) == 0) {
        auto wrapper = static_cast<PluginWrapper*>(p->plugin_data);
        if (wrapper->impl.guiIsApiSupported(nullptr, false)) {
            return &guiExtension;
        }
    }
    return nullptr;
