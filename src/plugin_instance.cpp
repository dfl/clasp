...
PluginInstance::PluginInstance(const PluginManifest& manifest)
    : manifest_(manifest)
    , paramCount_(manifest.parameters.size()) {
    // Initialize parameter storage using unique_ptr array (atomics aren't copyable)
    if (paramCount_ > 0) {
...
bool PluginInstance::activate(double sampleRate, uint32_t minFrames, uint32_t maxFrames) {
    if (!wasm_) return false;

    sampleRate_ = sampleRate;
    maxBlockSize_ = maxFrames;

    // Initialize the DSP
    if (!wasm_->init(static_cast<float>(sampleRate), static_cast<int>(maxFrames))) {
        return false;
    }
...
void PluginInstance::copyInputBuffers(const clap_process_t* process) {
    if (!process->audio_inputs || process->audio_inputs_count == 0) return;
...
void PluginInstance::copyOutputBuffers(const clap_process_t* process) {
    if (!process->audio_outputs || process->audio_outputs_count == 0) return;
...
clap_process_status PluginInstance::process(const clap_process_t* process) {
    ...
    for (const auto& note : pendingNoteOns_) {
        wasm_->noteOn(note.sampleOffset, note.noteId, note.channel,
                      note.key, note.velocity);
    }
    for (const auto& note : pendingNoteOffs_) {
        wasm_->noteOff(note.sampleOffset, note.noteId, note.channel,
                       note.key, note.velocity);
    }
