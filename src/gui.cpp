...
Gui::Gui(IPluginInstance *plugin, const PluginManifest &manifest)
    : plugin_(plugin), manifest_(manifest), width_(manifest.ui.width),
  ...
  return clasp_gui::WebView::isApiSupported(
      api == nullptr ? clasp_gui::WindowApi::Unknown :
                     ...
                     (strcmp(api, CLAP_WINDOW_API_COCOA) == 0
                          ? clasp_gui::WindowApi::Cocoa
                          : clasp_gui::WindowApi::Unknown)
#elif defined(_WIN32)
                     (strcmp(api, CLAP_WINDOW_API_WIN32) == 0
                          ? clasp_gui::WindowApi::Win32
                          : clasp_gui::WindowApi::Unknown)
#else
                     (strcmp(api, CLAP_WINDOW_API_X11) == 0
                          ? clasp_gui::WindowApi::X11
                          : clasp_gui::WindowApi::Unknown)
#endif
  );
}
...
bool Gui::create(const char *api, bool isFloating) {
  ...
  options.disableContextMenu = false; // Allow right-click for dev tools access

  webview_ = std::make_unique<clasp_gui::WebView>(options);
  if (!webview_->create())
    return false;

  // Connect WebView to logger for console forwarding
  Logger::instance().setWebView(webview_.get());
...
void Gui::setupBindings() {
...
#if CLASP_HAS_JSON
  // Register setParam handler - called from JS via clasp.call('setParam', id,
  // value)
  protocol_->onCall("setParam",
                    [this](const std::string &argsJson) -> std::string {
                      try {
  ...
  protocol_->onCall(
      "getPluginInfo", [this](const std::string &) -> std::string {
...
void Gui::destroy() {
  protocol_.reset();
  if (webview_) {
    // Disconnect from logger before destroying
    Logger::instance().setWebView(nullptr);
    webview_->destroy();
    webview_.reset();
  }
  visible_ = false;
...
bool Gui::adjustSize(uint32_t *width, uint32_t *height) { return true; }
...
bool Gui::setTransient(const clap_window_t *window) { return false; }
